    ...
    .filter(charge => {
      ...
      if (filterStatus) {
        if (filterStatus === 'atrasada') {
          matchesStatus = charge.status === 'pendente' && isVencido(charge.data_vencimento);
        } else if (filterStatus === 'pendente') {
          matchesStatus = charge.status === 'pendente' && !isVencido(charge.data_vencimento);
        } else {
          matchesStatus = charge.status === filterStatus;
        }
      }
      ...
      {showFilters && (
        <div className="glass-card rounded-2xl p-5 opacity-0 animate-fade-up">
          <div className="flex flex-wrap items-center gap-4">
            ...
            <div className="flex-1 min-w-[200px]">
              <Select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} options={[{value: '', label: 'Todos os status'}, {value: 'pendente', label: 'A Vencer'}, {value: 'pago', label: 'Pago'}, {value: 'atrasada', label: 'Atrasadas'}]} />
            </div>
            <div className="flex-1 min-w-[200px]">
